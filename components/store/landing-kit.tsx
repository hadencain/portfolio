...
export function AudienceLine({ children }: { children: React.ReactNode }) {
  return (
    ...
    >
      <p className="text-[10px] font-mono tracking-[0.3em] uppercase text-[#5c564a] mb-3">
        Who it&apos;s for
      </p>
      <p className="text-[15px] font-light text-[#a69f8f] leading-relaxed max-w-xl">
        {children}
      </p>
    </motion.div>
  );
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 gap-x-12 gap-y-10 mb-20 max-w-3xl">
      {beats.map((b, i) => (
        ...
        >
          <p className="text-[10px] font-mono tracking-[0.3em] uppercase text-[#665f51] mb-2">
            {b.name}
          </p>
          <p className="text-[12px] text-[#7d7566] font-light leading-relaxed">
            {b.text}
          </p>
        </motion.div>
      ))}
...
export function MediaSlot({ src, alt }: { src?: string; alt: string }) {
  ...
  return (
    <motion.div
      className="mb-20 max-w-3xl border border-[#262218] bg-[#12100c]"
